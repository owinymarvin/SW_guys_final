...
from huggingface_hub import hf_hub_download # New import for downloading from HF Hub

# Ensure torchvision and pytorchvideo are installed in your local environment
# pip install torch torchvision==0.13.1 pytorchvideo==0.1.5 opencv-python Pillow huggingface_hub
...
LOCAL_CACHE_DIR = "./hf_model_cache" # Directory to cache downloaded files locally
...
# Input to this pipeline will be (C, T, H, W) after subsampling
...
prediction_interval = 2 # Predict every 2 seconds (adjust as needed)
...
while True:
    ...
    # Perform prediction at a set interval if enough frames are collected
    if time.time() - last_prediction_time > prediction_interval and len(frame_buffer) == frames_needed_for_raw_clip:
        ...
        try:
            ...
            if predicted_id in id_to_label:
                current_prediction = id_to_label[predicted_id]
            else:
                current_prediction = f"Unknown ID: {predicted_id}"
            
            print(f"Predicted: {current_prediction} (Confidence: {preds[0, predicted_id].item():.2f})")
    ...
    # Display the frame with the current prediction
